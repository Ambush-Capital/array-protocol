...
#[derive(Accounts)]
pub struct InitDriftUserStats<'info> {
    #[account(mut)]
...
pub fn handle_init_drift_user_stats(ctx: Context<InitDriftUserStats>) -> Result<()> {
    ...
    let init_stats_ctx = CpiContext::new_with_signer(
    ...
    drift::cpi::initialize_user_stats(init_stats_ctx)?;
